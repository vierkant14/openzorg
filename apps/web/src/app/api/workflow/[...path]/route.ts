...
const WORKFLOW_URL = process.env.WORKFLOW_SERVICE_URL || "http://localhost:4003";
...
async function proxy(req: NextRequest, params: { path: string[] }): Promise<NextResponse> {
  ...
  const url = `${WORKFLOW_URL}/${path}${req.nextUrl.search}`;
  ...
  try {
    ...
    return NextResponse.json({ error: "Kan geen verbinding maken met de Workflow service" }, { status: 502 });
  }
}
